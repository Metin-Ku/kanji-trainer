...
export function LoginPage() {
  ...
  return (
    <div className="min-h-dvh max-w-md mx-auto flex flex-col justify-center px-6 py-10 bg-app-bg">
      <h1 className="text-2xl font-bold text-app-text mb-1">{t("auth.loginTitle")}</h1>
      <p className="text-sm text-app-text-secondary mb-6">{t("auth.loginSubtitle")}</p>
      ...
      <div className="mt-6 flex flex-col gap-2 text-sm text-center text-app-text-secondary">
        <Link href="/forgot-password" className="text-main-500 hover:underline">
          {t("auth.forgotPassword")}
        </Link>
        <p>
          {t("auth.noAccount")}{" "}
          <Link href="/register" className="text-main-500 hover:underline">
            {t("auth.register")}
          </Link>
        </p>
      </div>
