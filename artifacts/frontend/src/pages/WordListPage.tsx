...
import { Plus, ArrowUpDown, ArrowLeft, CheckSquare, Square, Copy, Trash2, Dices } from "lucide-react";
...
function buildComparator(sorts: Set<SortMode>): ((a: Word, b: Word) => number) | undefined {
  const levelDir = sorts.has("level-asc") ? "asc" : sorts.has("level-desc") ? "desc" : null;
  const dateDir  = sorts.has("date-asc")  ? "asc" : sorts.has("date-desc")  ? "desc" : null;
  const jlptDir  = sorts.has("jlpt-asc")  ? "asc" : sorts.has("jlpt-desc")  ? "desc" : null;
  if (!levelDir && !dateDir && !jlptDir) return undefined;
  return (a: Word, b: Word) => {
    if (jlptDir)  { const d = jlptRank(a) - jlptRank(b); if (d !== 0) return jlptDir === "asc" ? d : -d; }
    if (levelDir) { const d = a.level - b.level; if (d !== 0) return levelDir === "asc" ? d : -d; }
    if (dateDir)  { const d = new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime(); if (d !== 0) return dateDir === "asc" ? d : -d; }
    return 0;
  };
}
...
function toggleSort(prev: Set<SortMode>, value: SortMode): Set<SortMode> {
  ...
  const others = SORT_OPTIONS.filter((o) => o.group === opt.group && o.value !== value).map((o) => o.value);
  if (next.has(value)) { next.delete(value); } else { others.forEach((o) => next.delete(o)); next.add(value); }
  return next;
}
...
export function WordListPage() {
  ...
  const { words, isLoading, isError, addWord, updateWord, deleteWord, deleteWords, bulkCreate } = useWords();
  const [showForm, setShowForm] = useState(false);
  const [showBulk, setShowBulk] = useState(false);
  const [editingWord, setEditingWord] = useState<Word | undefined>(undefined);
  const [openIds, setOpenIds] = useState<Set<number>>(new Set());
  const [activeSorts, setActiveSorts] = useState<Set<SortMode>>(new Set<SortMode>(["level-asc"]));
  ...
  function handleSave(data: { kanji: string; pronunciation: string; meaning: string; description: string; level: number; jlptLevel: string | null; date: string; relatedWordIds: number[] }) {
    ...
    <div className="min-h-dvh bg-white">
        ...
        <div>
          {!isLoading && displayed.length === 0 && !isError ? (
            <div className="flex flex-col items-center justify-center py-24 px-8 text-center">
              {query ? (
                <><p className="text-4xl text-gray-200 mb-3">?</p><p className="text-gray-400 text-sm">"{query}" için sonuç bulunamadı</p></>
              ) : (
                <><div className="text-6xl mb-4 text-gray-200">漢</div><p className="text-gray-400 font-medium">Henüz kelime eklenmedi</p></>
              )}
            </div>
          ) : (
      ...
      {selectMode && (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 px-4 py-3 flex items-center gap-3">
          <button onClick={() => setSelectedIds(new Set(displayed.map((w) => w.id)))} className="text-xs text-gray-500 hover:text-gray-700 transition-colors shrink-0">
            Tümünü Seç
          </button>
          <span className="flex-1 text-center text-sm text-gray-500 font-medium">
            {selectedIds.size > 0 ? `${selectedIds.size} seçildi` : "Satır seçin"}
          </span>
          <button onClick={handleBulkDelete} disabled={selectedIds.size === 0} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-semibold text-white transition-opacity disabled:opacity-40 shrink-0" style={{ background: "rgb(239,68,68)" }}>
            <Trash2 size={14} />
            Sil
          </button>
        </div>
      ...
      {!selectMode && (
        <div ref={fabRef} className="fixed right-5 z-40" style={{ bottom: 32 }}>
          <div
            ...
            style={{
              background: "linear-gradient(160deg, rgb(255,160,40), rgb(255,80,5))",
              borderRadius: 999,
              width: 52,
              height: fabOpen ? 158 : 52,
              transition: "height 0.14s cubic-bezier(0.4,0,0.2,1)",
              boxShadow: "0 4px 20px rgba(255,100,10,0.4)",
              flexDirection: "column-reverse",
            }}
          >
            <button onClick={() => setFabOpen((v) => !v)} className="flex items-center justify-center active:opacity-70 transition-opacity" style={{ width: 52, height: 52, flexShrink: 0 }}>
              <div style={{ transform: fabOpen ? "rotate(45deg)" : "rotate(0deg)", transition: "transform 0.12s ease" }}>
                <Plus size={22} strokeWidth={2.5} className="text-white" />
              </div>
            </button>
            <div style={{ width: 28, height: 1, background: "rgba(255,255,255,0.3)", flexShrink: 0 }} />
            <button onClick={() => { setFabOpen(false); setEditingWord(undefined); setShowForm(true); }} className="flex items-center justify-center active:opacity-70 transition-opacity" style={{ width: 52, height: 52, flexShrink: 0 }}>
              <Plus size={22} strokeWidth={2.5} className="text-white" />
            </button>
            <div style={{ width: 28, height: 1, background: "rgba(255,255,255,0.3)", flexShrink: 0 }} />
            <button onClick={() => { setFabOpen(false); setShowBulk(true); }} className="flex items-center justify-center active:opacity-70 transition-opacity" style={{ width: 52, height: 52, flexShrink: 0 }}>
      ...
      {showForm && (
        <WordFormModal initial={editingWord} allWords={words} onSave={handleSave} onClose={() => { setShowForm(false); setEditingWord(undefined); }} />
      )}
      {showBulk && (
        <BulkImportModal onImport={bulkCreate} onClose={() => setShowBulk(false)} />
      )}
    </div>
  );
}
