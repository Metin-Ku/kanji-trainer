...
export function RelatedWordsSelect({ allWords, selectedIds, currentWordId, onChange }: Props) {
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    if ((e.key === "Backspace" || e.key === "Delete") && query === "" && selectedIds.length > 0) {
      e.preventDefault();
    ...
    <div ref={containerRef} className="relative">
      ...
      >
        {selectedWords.map((w) => (
          <span
            key={w.id}
            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-sm font-bold select-none"
